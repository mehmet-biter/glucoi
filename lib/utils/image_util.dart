  ...
  return InkWell(
      ...
      child: Container(
        ...
        child: imagePath.isValid
            ? imagePath!.contains(".svg")
                ? SvgPicture.network(imagePath, fit: boxFit, colorFilter: _getColorFilter(iconColor))
                : Image.network(imagePath, fit: boxFit, errorBuilder: (context, error, stackTrace) => const AppLogo())
            : const AppLogo(),
      ));
}
...
Future getImage(bool isGallery, Function(File, bool) onChoose, bool isCrop, {CameraDevice? camera}) async {
  XFile? res;
  if (isGallery) {
    res = await ImagePicker().pickImage(source: ImageSource.gallery);
  } else {
    res = await ImagePicker().pickImage(source: ImageSource.camera, imageQuality: 50, preferredCameraDevice: camera ?? CameraDevice.rear);
  }
  if (res != null) {
    if (isCrop) {
