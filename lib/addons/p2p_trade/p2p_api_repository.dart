...
class P2pAPIRepository {
  ...
  Future<ServerResponse> p2pPaymentMethodSave(P2pPaymentInfo paymentInfo) async {
    var mapObj = <String, dynamic>{};
    mapObj[P2pAPIKeyConstants.paymentUid] = paymentInfo.adminPaymentMethod?.uid;
    mapObj[P2pAPIKeyConstants.username] = paymentInfo.username;
    if (paymentInfo.uid.isValid) {
      mapObj[P2pAPIKeyConstants.uid] = paymentInfo.uid;
    }
    if (paymentInfo.adminPaymentMethod?.paymentType == P2pPaymentType.mobile) {
      mapObj[P2pAPIKeyConstants.mobileAccountNumber] = paymentInfo.mobileAccountNumber;
    } else if (paymentInfo.adminPaymentMethod?.paymentType == P2pPaymentType.card) {
      mapObj[P2pAPIKeyConstants.cardNumber] = paymentInfo.cardNumber;
      mapObj[P2pAPIKeyConstants.cardType] = paymentInfo.cardType;
    } else if (paymentInfo.adminPaymentMethod?.paymentType == P2pPaymentType.bank) {
      mapObj[P2pAPIKeyConstants.bankAccountNumber] = paymentInfo.bankAccountNumber;
      mapObj[P2pAPIKeyConstants.bankName] = paymentInfo.bankName;
      mapObj[P2pAPIKeyConstants.accountOpeningBranch] = paymentInfo.accountOpeningBranch;
      mapObj[P2pAPIKeyConstants.transactionReference] = paymentInfo.transactionReference;
    }
    return provider.postRequestFormData(P2pAPIURLConstants.p2pPaymentMethod, mapObj, APIRepository().authHeader(), isDynamic: true);
  }

  Future<ServerResponse> p2pPaymentMethodDelete(String uid) async {
    final mapObj = {};
    mapObj[P2pAPIKeyConstants.delete] = uid;
    return provider.postRequest(P2pAPIURLConstants.p2pPaymentMethod, mapObj, APIRepository().authHeader(), isDynamic: true);
  }
