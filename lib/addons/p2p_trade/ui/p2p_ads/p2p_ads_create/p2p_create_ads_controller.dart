...
class P2pCreateAdsController extends GetxController {
  ...
  final paymentTagController = TextfieldTagsController();
  ...
  final countryTagController = TextfieldTagsController();
  ...
  List<String> getTimeLimitList() {
    ...
    if ((adsSettings?.paymentTime ?? []).isValid) {
      list = adsSettings!.paymentTime!.map((e) => "${e.time ?? 0} ${"minutes".tr}").toList();
    }
  ...
  List<String> getPaymentNameList() {
    if ((adsSettings?.paymentMethods ?? []).isValid) {
      return adsSettings!.paymentMethods!.map((e) => e.adminPaymentMethod?.name ?? "").toList();
    }
    return [];
  }
  ...
  void saveOrEditAds(BuildContext context) async {
    ...
    try {
      ...
      if (resp.success) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((timeStamp) => showToast(message, isError: !success, context: context, isLong: true));
        if (success) {
          Get.back();
