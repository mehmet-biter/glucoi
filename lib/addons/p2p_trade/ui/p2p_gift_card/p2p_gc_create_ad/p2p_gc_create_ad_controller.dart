...
class P2pGCCreateAdController extends GetxController {
  ...
  void setPreAdData() {
    if (!isEdit && settings.assets.isValid) {
      ...
      for (final uid in payIdList) {
        final payment = settings.paymentMethods?.firstWhere((element) => element.uid == uid).adminPaymentMethod?.name;
  ...
  List<String> getTimeLimitList() {
    ...
    if (settings.paymentTime.isValid) {
      list = settings.paymentTime!.map((e) => "${e.time ?? 0} ${"minutes".tr}").toList();
    }
