...
import 'gift_cards_details/gift_cards_details_screen.dart';
...
class GiftBannerItemView extends StatelessWidget {
  const GiftBannerItemView({super.key, required this.gBanner, this.isSelected = false, this.isPriceShow = false, this.onTap});
      ...
      children: [
        ...
        if (isPriceShow)
          Positioned(
            bottom: 0,
            right: 5,
            child: buttonText("${gBanner.minRecipientAmount} ${gBanner.recipientCurrency ?? ""}",
                visualDensity: minimumVisualDensity, fontSize: Dimens.regularFontSizeMin),
          )
      ],
    );
  }
...
class GiftCardItemView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      onTap: () {
        showBottomSheetFullScreen(context, GiftCardDetailsScreen(gCard: gCard), title: "Gift Card Details".tr);
      },
      ...
      child: Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            showImageNetwork(imagePath: gCard.banner?.banner, width: cWidth, height: cWidth / 2, boxFit: BoxFit.cover),
            vSpacer5(),
            textAutoSizeKarla(gCard.banner?.title ?? "", textAlign: TextAlign.start, fontSize: Dimens.regularFontSizeMid, maxLines: 2),
            vSpacer5(),
            textAutoSizePoppins("${coinFormat(gCard.payAmount)} ${gCard.coinType ?? ""}", textAlign: TextAlign.start)
          ],
        ),
      ),
    );
