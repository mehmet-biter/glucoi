...
class FiatWithdrawalPageState extends State<FiatWithdrawBankPage> {
    ...
    return Column(
      ...
      children: [
        ...
        textFieldWithWidget(
            ...
            suffixWidget: Obx(() => walletsSuffixView(_controller.fiatWithdrawalData.value.myWallet ?? [], selectedWallet.value,
                onChange: (selected) => selectedWallet.value = selected))),
        Obx(() {
          return selectedWallet.value.coinType.isValid
              ? textAutoSizePoppins(
                  "withdraw_Max_min".trParams(
                      {"min": coinFormat(selectedWallet.value.minimumWithdrawal), "max": coinFormat(selectedWallet.value.maximumWithdrawal)}),
                  maxLines: 2,
  ...
  void _checkInputData() {
    ...
    if(!CurrencyCheck.checkWithdrawCurrency(selectedWallet.value.coinType, context)){
      return;
    }
    ...
    //   return;
    // }
    final minAmount = selectedWallet.value.minimumWithdrawal ?? 0;
    if (amount < minAmount) {
      showToast("Amount_less_then".trParams({"amount": minAmount.toString()}));
      return;
    }
    final maxAmount = selectedWallet.value.maximumWithdrawal ?? 0;
    if (amount > maxAmount) {
    ...
    final subTitle = "${"You will withdrawal".tr} $amount ${selectedWallet.value.coinType ?? ""} $payInfo";
    WithdrawConfirmHelper().checkWithdrawToConfirmFiat(context, subTitle, (code) {
      final withdraw = CreateWithdrawal(
          walletId: selectedWallet.value.encryptId, currency: selectedWallet.value.coinType, amount: amount, bankId: bankId, paymentInfo: payInfo);
