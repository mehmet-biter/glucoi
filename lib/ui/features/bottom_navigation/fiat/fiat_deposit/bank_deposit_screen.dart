...
class _BankDepositScreenState extends State<BankDepositScreen> {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        vSpacer5(),
        Obx(() {
          return dropDownListIndex(_controller.getBankList(_controller.fiatDepositData.value), selectedBankIndex.value, "Select Bank".tr,
  ...
  void _checkInputData() {
    ...
    final deposit = CreateDeposit(
        walletId: selectedWallet.value.id, amount: amount, bankId: bank?.id, file: selectedFile.value, currency: selectedWallet.value.coinType ?? "");
