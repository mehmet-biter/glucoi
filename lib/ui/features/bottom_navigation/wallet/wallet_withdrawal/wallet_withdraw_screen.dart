...
class _WalletWithdrawScreenState extends State<WalletWithdrawScreen> {
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      textAutoSizePoppins("only_enter_valid_address_withdraw".trParams({"coin": walletRx.value.coinType ?? ""}),
                          color: Colors.redAccent, maxLines: 2, textAlign: TextAlign.start),
                      vSpacer10(),
                      textFieldWithSuffixIcon(
                      ...
                      vSpacer2(),
                      textAutoSizePoppins("ensure_memo_correct".tr, maxLines: 2, textAlign: TextAlign.start),
                      vSpacer10(),
                      Obx(() {
  ...
  void _setViewData(Map data) {
    ...
    if (getSettingsLocal()?.isEvmWallet ?? false) {
      ...
      if (networkList.isNotEmpty) {
        ...
        final networkList = data[APIKeyConstants.wallet]?[APIKeyConstants.coin]?["coin_network"] as List?;
        if (networkList.isValid) {
          final network = networkList!.first?["network"];
          if (network != null) selectedNetwork.value = Network.fromJson(network);
        }
      }
  ...
  _networkView() {
    return Obx(() => networkList.isNotEmpty
        ? Column(
            ...
            children: [
              vSpacer10(),
              textAutoSizeKarla("Select Network".tr, fontSize: Dimens.regularFontSizeMid, textAlign: TextAlign.start),
              vSpacer5(),
  ...
  void _checkInputData() {
    ...
    WithdrawConfirmHelper().checkWithdrawToConfirmCrypto(context, subTitle, (code) => _callWithdrawProcess(address, amount, code));
  }

  void _callWithdrawProcess(String address, double amount, String code) {
    final memo = _memoEditController.text.trim();
    if (getSettingsLocal()?.isEvmWallet ?? false) {
      selectedNetwork.value.baseType = baseNetworkType.value;
      _controller.withdrawProcessEvm(
        widget.wallet,
        address,
        amount,
        code,
        ...
        (message) {
          ...
          Future.delayed(const Duration(seconds: 5), () => _controller.getHistoryListData(HistoryType.withdraw, (list) => historyList.value = list));
        },
      );
    } else {
      _controller.withdrawProcess(widget.wallet, address, amount, selectedNetwork.value.networkType ?? "", code, memo);
    }
  }
