...
class _WalletDepositScreenState extends State<WalletDepositScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      _controller.getWalletDeposit(widget.wallet.id, (value) {
    ...
    if (getSettingsLocal()?.isEvmWallet ?? false) {
      ...
      if ([NetworkType.trc20Token, NetworkType.evmBaseCoin].contains(baseNetworkType.value)) {
        ...
        if (networkList.isNotEmpty) {
          ...
          if(adList.isNotEmpty) selectedAddress.value = adList.first.address ?? "";
        }
      }
    } else {
  ...
  //         children: [
  //           vSpacer10(),
  //           if (networkList.isNotEmpty) textAutoSizeKarla("Select Network".tr, fontSize: Dimens.regularFontSizeMid, textAlign: TextAlign.start),
  //           vSpacer5(),
  //           if (networkList.isNotEmpty)
  //             dropDownNetworks(networkList, selectedNetwork.value, "Select".tr, onChange: (value) {
  //               selectedNetwork.value = value;
  ...
  Widget _memoView() {
    return Obx(() => selectedMemo.isNotEmpty
        ? Column(
            ...
            children: [
              vSpacer10(),
              textAutoSizeKarla("Memo".tr, fontSize: Dimens.regularFontSizeMid, textAlign: TextAlign.start),
              vSpacer5(),
