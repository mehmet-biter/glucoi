...
class FiatWithdrawal {
  ...
  factory FiatWithdrawal.fromJson(Map<String, dynamic> json) => FiatWithdrawal(
        myWallet: (json["my_wallet"] ?? json["wallet_list"]) == null
            ? null
            : List<Wallet>.from((json["my_wallet"] ?? json["wallet_list"]).map((x) => Wallet.fromJson(x))),
...
class CreateDeposit {
  ...
  Future<Map<String, dynamic>> toJson() async {
    ...
    if (transactionId.isValid) mapObj[APIKeyConstants.transactionId] = transactionId;
    if (code.isValid) mapObj[APIKeyConstants.code] = code;
    if (email.isValid) mapObj[APIKeyConstants.email] = email;
    return mapObj;
  }
}
