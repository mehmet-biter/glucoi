...
import '../ui/features/bottom_navigation/wallet/wallet_deposit/wallet_deposit_controller.dart';
import 'app_widgets.dart';
import 'kyc_verify_need_page.dart';
...
List getHistoryTypeData(String type) {
  switch (type) {
    ...
    case HistoryType.swapBuyCrypto:
      return ["Buy Crypto", Colors.blueAccent];
    case HistoryType.swapSellCrypto:
      return ["Sell Crypto", Colors.lightBlue];
...
FiatDepositController getFiatDepositController() {
  if (Get.isRegistered<FiatDepositController>()) {
    return Get.find<FiatDepositController>();
  } else {
    return Get.put(FiatDepositController());
  }
}

WalletDepositController getWalletDepositController() {
  if (Get.isRegistered<WalletDepositController>()) {
    return Get.find<WalletDepositController>();
  } else {
    return Get.put(WalletDepositController());
  }
}
...
List getIdVerificationStatusData(String? status) {
...
void setKYCVerified(KycSettings? kyc) {
  if (kyc != null) {
    if (kyc.enabledKycType == KYCType.persona) {
      gIsKYCVerified = kyc.enabledKycUserDetails?.persona?.isVerified == 1;
    } else if (kyc.enabledKycType == KYCType.dojah) {
      gIsKYCVerified = kyc.enabledKycUserDetails?.dojah?.isVerified == 1;
    } else if (kyc.enabledKycType == KYCType.manual) {
      final kDetails = kyc.enabledKycUserDetails;
      if (kDetails is KycDetails) {
        if (kDetails.nid?.status == IdVerificationStatus.accepted) gIsKYCVerified = true;
        if (kDetails.passport?.status == IdVerificationStatus.accepted) gIsKYCVerified = true;
        if (kDetails.driving?.status == IdVerificationStatus.accepted) gIsKYCVerified = true;
        if (kDetails.voter?.status == IdVerificationStatus.accepted) gIsKYCVerified = true;
      }
    }
    GetStorage().write(PreferenceKey.isKycVerified, gIsKYCVerified);
  }
}

void checkKYCVerifyStatus(BuildContext context, {VoidCallback? onCancel}) {
  Future.delayed(const Duration(seconds: 2), () {
    if (!gIsKYCVerified && context.mounted) {
      showModalBottomSheet(
          ...
          builder: (BuildContext context) => KycVerifyNeedPage(onCancel: onCancel));
    }
  });
}

void checkLoggedInAndKYCVerifyStatus(BuildContext context, Function() onSuccess) {
  if (gUserRx.value.id == 0) {
    showModalSheetFullScreen(context, signInNeedView(isDrawer: true));
  } else if (!gIsKYCVerified && context.mounted){
    showModalBottomSheet(
        context: context,
        isScrollControlled: true,
        enableDrag: false,
        isDismissible: false,
        backgroundColor: Colors.transparent,
        builder: (BuildContext context) => const KycVerifyNeedPage());
  }else {
    onSuccess();
  }
}
